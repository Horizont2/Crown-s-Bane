...
void AShipPawn::UpdateVisualRoll(float DeltaTime)
{
	...
	// Passive wave rocking driven by UPROPERTY amplitudes.  Set to 0 for calm sea.
	const float T = GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0;
	const float WavePitch = FMath::Sin(T * 0.6f) * WavePitchAmplitude;
	const float WaveRoll  = FMath::Sin(T * 0.4f) * WaveRollAmplitude;
	const float WaveBob   = FMath::Sin(T * 0.5f) * WaveBobAmplitude;
	...
	LocalLoc.Z = FMath::FInterpTo(LocalLoc.Z, WaterLineOffset + WaveBob, DeltaTime, 3.0f);
